...
def make_animation(source_image, driving_video, model, kp_detector, cfg, max_num_pixels, relative=False, adapt_movement_scale=False):
    ...
    with torch.no_grad():
        ...
        for frame_idx in tqdm(range(driving.shape[2])):
            ...
            predictions.append(torch.clamp(out[0], 0., 1.).cpu())
    return predictions
...
if __name__ == "__main__":
    ...
    parser.add_argument("--absolute", action="store_true", help="Use absolute motion transfer")
    ...
    kp_detector.load_state_dict(torch.load('./checkpoints/kp_detector.pt'))
